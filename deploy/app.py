...
def create_app():
    ...
    w3 = None
    if Web3 is not None and provider:
        try:
    ...
    source = None
    with open("CertificateVerification.sol", "r", encoding="utf-8") as f:
        source = f.read()
    ...
    def ensure_contract():
        ...
        if contract_address:
            ...
            return contract
        if os.getenv("VERCEL") or compile_source is None or install_solc is None:
            return None
        install_solc("0.8.20")
    ...
    def issue():
        ...
        tx_hash_hex = None
        addr = None
        if c is not None and Web3 is not None:
    ...
    @app.route("/api/admin/deploy_contract", methods=["POST"])
    @jwt_required()
    def deploy_contract():
        claims = get_jwt()
        if claims.get("role") != "issuer":
        ...
        c = ensure_contract()
        if c is None:
            return jsonify({"error": "deploy_failed"}), 500
        return jsonify({"contract": c.address})
